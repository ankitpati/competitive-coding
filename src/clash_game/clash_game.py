...
def main():
    ...
    raw_objs, tests = literal_eval(input()), literal_eval(input())
    objects = []
    beads = {}

    for ro in raw_objs:
        if ro[0] == 'Bead':
            objects.append(getattr(modules[__name__], ro[0])(*ro[1]))
            beads[objects[-1].id] = objects[-1]
        else:
            objects.append(Board(*ro[1][0:2], list(beads.values())))
    ...
    for method, args in tests:
        ...
        if method == 'get_beads':
            bs = [bead.id for bead in obj.get_beads()]
            bs.sort()
            returns.append(tuple(bs))
        ...
        elif method == 'get_random_unoccupied_cell':
            cell = obj.get_random_unoccupied_cell()
            returns.append((0 <= cell[0] < obj.width, 0 <= cell[1] < obj.height))

        elif method == 'slide_to':
            bead = beads[args[1]]
            old_cell = bead.cell
            dest = args[2]
            obj.slide_to(bead, dest)
            returns += [
                bead.get_cell() == dest, dest in obj.cells_with_beads,
                old_cell not in obj.cells_with_beads,
                bead.id in obj.cells_with_beads[dest]
            ]

        elif method == 'perform_collision':
            bs = [beads[id] for id in args[1]]
            pre_life = [bead.life for bead in bs]
            obj.perform_collision(args[1])
            after_life = [bead.life for bead in bs]

            ret = True
            for (pl, al) in zip(pre_life, after_life):
                if pl - 1 != al:
                    ret = False

            returns.append(ret)

        elif method == 'manage_collisions':
            bs = [beads[3], beads[4]]
            pre_life = [bead.life for bead in bs]
            obj.manage_collisions()
            after_life = [bead.life for bead in bs]
